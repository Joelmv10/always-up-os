# Plantillas de outreach — clubes objetivo

> Para usar con la lista de [pipeline.md](../01-clients/pipeline.md). Ver [playbook-ventas.md](playbook-ventas.md) para contexto de pricing/objeciones. **No repetir el enfoque anterior de Joel con contactos NCAA (mensaje directo en frío, sin respuesta)** — esta secuencia parte de identificar primero a la persona correcta (entrenador/director deportivo) y usa la prueba social real desde el primer mensaje.

## Mensaje 1 — primer contacto (email o redes)

Asunto: *Bringing your club to train with professional Spanish clubs*

```
Hi [Name],

My name is Joel Martinez, I run Always Up — we organize football experiences
in Spain for international youth clubs, training directly at the facilities
of professional La Liga clubs (Atlético de Madrid, Real Sociedad, Sporting
de Gijón).

I came across [Club Name] and thought this could be a good fit given [specific
reason — e.g. "your team's participation in the Costa Daurada Cup" / "your
club's track record of European trips"].

One of our long-term partner clubs from Ireland recently won their first
league title in the club's history, and credited part of that to the training
environment and mentality they experienced during their time with us in Spain.

If bringing your players closer to professional football is something you'd
consider for [next season/summer], I'd love to share more — happy to send
over details or jump on a quick call, whichever is easier for you.

Best,
Joel
```

## Mensaje 2 — seguimiento (si no hay respuesta en ~10-14 días)

```
Hi [Name],

Just wanted to follow up in case this got buried — no pressure at all, I know
these decisions take time and planning.

If it's useful, happy to send a short one-page overview of what a typical
experience looks like (training, matches, facilities), so you can share it
internally with your staff/board before deciding if it's worth a call.

Best,
Joel
```

## Notas de uso

- **Personalizar siempre el motivo de contacto** — no enviar el mensaje 1 genérico sin el `[specific reason]` relleno. Si no hay un motivo real y específico, no está listo para contactar todavía.
- **Priorizar salida vía referencia** cuando exista (ej. pedir a Greystones que presente/recomiende a otro club irlandés) antes que el contacto totalmente frío.
- Idioma: inglés por defecto (mercado objetivo es internacional) — versión en español solo si el club es español.
- Cadencia: mensaje 1 → esperar 10-14 días → mensaje 2 → si tampoco hay respuesta, pasar a la siguiente cuenta de la lista sin insistir más (no hay señal de interés real).
